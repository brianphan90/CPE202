class LinkedList:
    ...
    def __contains__(self,item):
        first_item = self.first.setItem(item)
        cur_node = self.first.getNext()
        if cur_node.getItem() == self.first.getItem():
            return True
        while cur_node.getitem() != item:
    ...
    def __delitem__(self,index):
        cur_node = self.first.getNext()        #in the case that the head node is deleted
        if index == 0:
            self.first = cur_node.getNext() #sets the head to the next node
            cur_node = None
            return 
           
        count = 1
        prev_node = None
        while cur_node and count != index:  #checks if cur node is at the index
            prev_node = cur_node
            cur_node = cur_node.getNext()
            count += 1 
        if cur_node is None:            #index not in linked list
            print("index is not in linked list")
            
        prev_node.getNext = cur_node.Next
        cur_node.Next = None
